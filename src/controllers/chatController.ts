...
// --- WISPHUB ACTIVATE ---
  ...
  async function processPostAuthActions(data: any, targetId?: string | number) {
      ...
      console.log(`[processPostAuthActions] Fuentes posibles activarInstalacionGeonet: data.usuario='${data.usuario}', session.lastSelectedUsuarioInstalacion='${sessionUsuarioInstalacion}', session.lastAuthNameUsed='${data._session && data._session.lastAuthNameUsed}', session dump=${sessionDump}`);
      ...
      if (!skipGeonetRegistration) {
        ...
        if (clientid !== undefined) {
          ...
          try {
            ...
            if (entity && entity.usuario) {
              ...
              if (entity && entity.usuario) {
                ...
                clienteUsuario = data.usuario || sessionUsuario || `${clientid}@geonet`;
                if (data.usuario) clienteUsuarioSource = 'data.usuario';
                else if (sessionUsuario) clienteUsuarioSource = 'session.lastAuthNameUsed';
          ...
          console.log(`[processPostAuthActions] Fuentes posibles: data.usuario='${data.usuario}', session.lastAuthNameUsed='${sessionUsuario}', session dump=${sessionDump}`);
